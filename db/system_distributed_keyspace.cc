...
future<> system_distributed_keyspace::insert_snapshot_sstable(sstring snapshot_name, sstring ks, sstring table, sstring dc, sstring rack, sstables::sstable_id sstable_id, dht::token first_token, dht::token last_token, sstring toc_name, sstring prefix, bool downloaded, db::consistency_level cl) {
    ...
    return _qp.execute_internal(
            ...
            { std::move(snapshot_name), std::move(ks), std::move(table), std::move(dc), std::move(rack),
              dht::token::to_int64(first_token), sstable_id.uuid(), dht::token::to_int64(last_token), std::move(toc_name), std::move(prefix), downloaded },
...
future<> system_distributed_keyspace::update_sstable_download_status(sstring snapshot_name,
    ...
    co_await _qp.execute_internal(update_query,
                                  ...
                                  internal_distributed_query_state(),
                                  {downloaded, snapshot_name, ks, table, dc, rack, dht::token::to_int64(start_token), sstable_id.uuid()},
                                  cql3::query_processor::cache_internal::no);
}
