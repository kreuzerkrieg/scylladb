...
public:
    ...
    future<sstables::data_source_creator_fn> wrap_source(const sstables::sstable& sst,
        ...
        case sstables::component_type::Unknown:
            ...
            if (esx) {
                auto k = co_await esx->key_for_read(std::move(id));
                data_source_creator = [data_source_creator = std::move(data_source_creator), key = std::move(k)](uint64_t offset, uint64_t len) {
                    auto block_size = key->block_size();
                    return data_source(make_encrypted_source(data_source_creator(align_down(offset, block_size), align_up(len, block_size)), std::move(key)));
                };
            }
            co_return data_source_creator;
        }
