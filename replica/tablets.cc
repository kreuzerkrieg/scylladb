...
tablet_id process_one_row(replica::database* db, table_id table, tablet_map& map, tablet_id tid, const cql3::untyped_result_set_row& row) {
    ...
    map.set_tablet(tid, tablet_info{std::move(tablet_replicas), repair_time, repair_task_info, migration_task_info, sstables_repaired_at, restore_cfg});
...
future<table_id> recreate_table_with_tablet_hints(const sstring& snapshot_name, const sstring& ks, const table_id& tid,
    ...
    co_await sys_dis_ks.insert_snapshot_cql_table(snapshot_name, ks, cf_name, schema->is_view(), create_stmt, cl);
    ...
    auto execute_ddl = [&] (ddl_operation op) -> future<> {
        while (true) {
            ...
            sstring description = format("Dropping and recreating table {}.{} with tablet count hints min_tablet_count={} max_tablet_count={}", ks, cf_name, min_tablet_count, max_tablet_count);

            utils::chunked_vector<mutation> mutations;
            if (op == ddl_operation::drop) {
                mutations = co_await service::prepare_column_family_drop_announcement(sp, ks, cf_name, ts, service::drop_views::yes); // drop_views = true ?
            } else {
