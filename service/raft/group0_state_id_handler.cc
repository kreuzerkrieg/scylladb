...
static logging::logger g0sih_logger("group0_tombstone_gc_handler");
...
void group0_state_id_handler::refresh() {
    ...
    if (group0_members.empty()) {
        g0sih_logger.info("Skipping due to empty group0");
        return;
    }

    g0sih_logger.trace("Current node count: {}", group0_members.size());
    ...
    if (!group0_members_missing_endpoint.empty()) {
        g0sih_logger.info("Skipping due to missing endpoints for members: {}", fmt::join(group0_members_missing_endpoint, ", "));
        return;
    }

    if (!group0_members_missing_state_id.empty()) {
        g0sih_logger.info("Skipping due to missing state id of some endpoints: {}", fmt::join(group0_members_missing_state_id, ", "));
        return;
    }
    ...
    const auto min_state_id = std::ranges::min(group0_members_state_ids, [](auto a, auto b) {
        if (!a || !b) {
            ...
            on_internal_error(g0sih_logger, "unexpected empty state_id");
        }
    ...
    if (_state_id_last_reconcile && utils::timeuuid_tri_compare(_state_id_last_reconcile, min_state_id) > 0) {
        g0sih_logger.info("Skipping due to the stale minimum state id: {}", min_state_id);
        return;
    }
    ...
    g0sih_logger.info("Setting reconcile time to {} (min id={})", tombstone_gc_time, min_state_id);
...
void group0_state_id_handler::run() {
    if (this_shard_id() != 0) {
        on_fatal_internal_error(g0sih_logger, "group0_state_id_handler must be started on shard 0");
    }
...
future<> group0_state_id_handler::advertise_state_id(utils::UUID state_id) {
    if (!_gossiper.is_enabled()) {
        g0sih_logger.debug("Skipping advertisement of state id {} because gossiper is not active", state_id);
        return make_ready_future();
    }

    if (_state_id_last_advertised && utils::timeuuid_tri_compare(_state_id_last_advertised, state_id) > 0) {
        g0sih_logger.debug("Skipping advertisement of stale state id {}", state_id);
        return make_ready_future();
    }
    ...
    g0sih_logger.debug("Advertising state id: {} (gc_time: {})", state_id, gc_time);
