...
void storage_service::init_messaging_service() {
    ...
    ser::streaming_rpc_verbs::register_clone_sstable(&_messaging.local(),
        ...
        co_return co_await container().invoke_on(0, [req] (storage_service& ss) -> future<streaming::stream_files_response> {
            co_return co_await streaming::clone_sstable_handler(ss._db.local(), ss._view_building_worker.local(), req);
        });
    });
