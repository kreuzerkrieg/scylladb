...
class index_reader {
    ...
    std::unique_ptr<index_consume_entry_context<index_consumer>> make_context(uint64_t begin, uint64_t end, index_consumer& consumer) {
        ...
        auto input = input_stream<char>(_sstable->get_storage().make_data_or_index_source(
            *_sstable, component_type::Index, begin, (_single_page_read ? end : _sstable->index_size()) - begin, get_file_input_stream_options()));
