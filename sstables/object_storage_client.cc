...
static shared_ptr<s3::client> make_s3_client(const db::object_storage_endpoint_param& ep, semaphore& memory, std::function<shared_ptr<s3::client>(std::string)> factory, double connections_per_share) {
    auto& epc = ep.get_s3_storage();
    return s3::client::make(epc.endpoint, epc.region, epc.iam_role_arn, memory, std::move(factory), connections_per_share);
}
...
public:
    s3_client_wrapper(const db::object_storage_endpoint_param& ep, semaphore& memory, shard_client_factory cf, double connections_per_share)
        : _client(make_s3_client(ep, memory, std::bind_front(&s3_client_wrapper::shard_client, this), connections_per_share))
        , _cf(std::move(cf))
    {}
...
shared_ptr<object_storage_client> sstables::make_object_storage_client(const db::object_storage_endpoint_param& ep, semaphore& memory, shard_client_factory cf, double s3_connections_per_share) {
    if (ep.is_s3_storage()) {
        return seastar::make_shared<s3_client_wrapper>(ep, memory, std::move(cf), s3_connections_per_share);
    }
    if (ep.is_gs_storage()) {
