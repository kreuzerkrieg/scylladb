...
static future<data_source> maybe_wrap_source(const sstable& sst, component_type type, data_source_creator_fn source_creator, uint64_t offset, uint64_t len) {
    if (type != component_type::TOC && type != component_type::TemporaryTOC) {
        for (auto* ext : sst.manager().config().extensions().sstable_file_io_extensions()) {
            std::exception_ptr p;
            try {
                source_creator = co_await ext->wrap_source(sst, type, std::move(source_creator), offset, len);
            } catch (...) {
                p = std::current_exception();
            }
            if (p) {
...
s3_storage::make_data_or_index_source(sstable& sst, component_type type, file, uint64_t offset, uint64_t len, file_input_stream_options) const {
    return maybe_wrap_source(
        sst,
        type,
        [this, object_name = make_s3_object_name(sst, type)](uint64_t offset, uint64_t len) {
            return _client->make_chunked_download_source(object_name, s3::range{offset, len}, _as);
        },
        offset,
        len);
}
