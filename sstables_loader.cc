...
public:
    download_task_impl(tasks::task_manager::module_ptr module,
                       sharded<sstables_loader>& loader,
                       sstring endpoint,
                       sstring bucket,
                       sstring ks,
                       sstring cf,
                       sstring prefix,
                       std::vector<std::string>&& sstables) noexcept
...
future<> sstables_loader::download_task_impl::run() {
    ...
    auto [ table_id, sstables_on_shards ] = co_await replica::distributed_loader::get_sstables_from_object_store(_loader.local()._db, _ks, _cf, _endpoint, _bucket, _prefix, cfg);
...
future<tasks::task_id> sstables_loader::download_new_sstables(
    sstring ks_name, sstring cf_name, sstring endpoint, sstring bucket, sstring prefix, std::vector<std::string>&& sstables) {
    ...
    auto task = co_await _task_manager_module->make_and_start_task<download_task_impl>({}, container(), std::move(endpoint), std::move(bucket), std::move(ks_name), std::move(cf_name), std::move(prefix));
    co_return task->id();
}
