...
future<> sstables_loader::download_tablet_sstables(locator::global_tablet_id tid, sstring snap_name, sstring endpoint, sstring bucket) {
    ...
    // Group entries by S3 prefix since each backup node stores SSTables under
    // its own prefix, and get_sstables_from_object_store accepts a single prefix.
    std::map<sstring, std::vector<sstring>> toc_names_by_prefix;
...
static future<size_t> process_manifest(input_stream<char>& is, sstring keyspace, sstring table,
    ...
    if (keyspace.empty()) {
        keyspace = sstring(rjson::to_string_view(parsed["table"]["keyspace_name"]));
    }
    if (table.empty()) {
        table = sstring(rjson::to_string_view(parsed["table"]["table_name"]));
    }
...
future<tasks::task_id> sstables_loader::restore_tablets(table_id tid, sstring keyspace, sstring table, sstring snap_name, sstring endpoint, sstring bucket, utils::chunked_vector<sstring> manifests) {
    ...
    co_await replica::recreate_table_with_tablet_hints(snap_name, keyspace, tid, tablet_count, tablet_count, _db.local(), _sys_dist_ks, _sys_dist_ks.get_storage_proxy(), _sys_dist_ks.get_migration_manager());
