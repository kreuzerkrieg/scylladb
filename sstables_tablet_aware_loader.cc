...
future<sstables_tablet_aware_loader::minimal_sst_info> sstables_tablet_aware_loader::download_sstable(sstables::shared_sstable sstable) const {
    ...
    for (auto it = components.cbegin(); it != components.cend(); ++it) {
        _as_per_shard[this_shard_id()].check();
        try {
...
future<utils::chunked_vector<db::sstable_info>> sstables_tablet_aware_loader::get_owned_sstables() const {
    ...
    for (const auto& tablet_range : tablets_in_scope) {
        auto sst_infos = co_await _loader.get_systesm_distributed_keyspace().local().get_snapshot_sstables(
            _snapshot, _keyspace, _table, _data_center, _rack, db::consistency_level::LOCAL_ONE, std::nullopt, tablet_range.end().value().value());
        tall.debug("{} SSTables found for tablet {}", sst_infos.size(), tablet_range);
...
future<std::vector<std::vector<sstables_tablet_aware_loader::minimal_sst_info>>> sstables_tablet_aware_loader::get_snapshot_sstables() const {
    ...
    auto owned_sstables = co_await get_owned_sstables();
    ...
    auto [_, sstables_on_shards] = co_await replica::distributed_loader::get_sstables_from_object_store(
        ...
        [&] { return &_as_per_shard[this_shard_id()]; });
    ...
    co_await smp::invoke_on_all([this, &sstables_on_shards, &min_infos] -> future<> {
        ...
        _progress_per_shard[this_shard_id()].total = sst_chunk.size();
...
sstables_tablet_aware_loader::sstables_tablet_aware_loader(tablet_aware_loader& loader,
                                                           ...
                                                           std::vector<tasks::task_manager::task::progress>& progress_per_shard,
                                                           std::vector<abort_source>& as_per_shard)
    : _snapshot(snapshot)
    ...
    , _endpoint(endpoint)
    , _bucket(bucket)
    , _progress_per_shard(progress_per_shard)
    , _as_per_shard(as_per_shard)
    , _loader(loader) {
}
...
future<> sstables_tablet_aware_loader::load_snapshot_sstables() {
    try {
        ...
        co_await smp::invoke_on_all([this, &downloaded_ssts, from = this_shard_id()] -> future<> {
            ...
            co_await max_concurrent_for_each(shard_ssts, 16, [this, from](const auto& min_info) -> future<> {
                co_await attach_sstable(from, min_info);
                ++_progress_per_shard[this_shard_id()].completed;
            });
        });
    } catch (...) {
