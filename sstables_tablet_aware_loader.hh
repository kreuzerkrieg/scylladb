...
class sstables_tablet_aware_loader {
    ...
    std::vector<tasks::task_manager::task::progress>& _progress_per_shard;
    std::vector<abort_source>& _as_per_shard;
    tablet_aware_loader& _loader;

    future<minimal_sst_info> download_sstable(sstables::shared_sstable sstable) const;
    future<utils::chunked_vector<db::sstable_info>> get_owned_sstables() const;
