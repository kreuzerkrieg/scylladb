...
SEASTAR_FIXTURE_TEST_CASE(test_gcp_storage_download_source_resumes_after_failure, local_gcs_wrapper, *check_gcp_storage_test_enabled()) {
    ...
    // Drain the whole first range before injecting. The source fetches 8 MiB at a
    // time and serves later reads out of that buffer without going near the
    // network, so a failure injected before it is exhausted never fires.
    size_t read = 0;
