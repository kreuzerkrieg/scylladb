...
SEASTAR_THREAD_TEST_CASE(test_throttling_controller_freeze_stops_admission_on_throttle) {
    ...
    // It must lift on its own: the freeze is at most 5s.
...
SEASTAR_THREAD_TEST_CASE(test_throttling_controller_metrics_change_on_upload_proxy) {
    ...
    // The freeze counter must be exposed for the write operation too. It is not
    // asserted above zero: freeze_min_gap bounds how often a freeze may trip, so
    // a short test can legitimately see throttles without one.
