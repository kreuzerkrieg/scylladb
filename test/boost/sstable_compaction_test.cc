...
void tombstone_purge(test_env& env) {
    BOOST_REQUIRE(smp::count == 1);
...
SEASTAR_TEST_CASE(tombstone_purge_test) {
    return test_env::do_with_async([](test_env& env) { tombstone_purge(env); });
}

SEASTAR_TEST_CASE(tombstone_purge_s3_test, *boost::unit_test::precondition(tests::has_scylla_test_env)) {
    return test_env::do_with_async([](test_env& env) { tombstone_purge(env); }, test_env_config{.storage = make_test_object_storage_options("S3")});
}
...
SEASTAR_FIXTURE_TEST_CASE(tombstone_purge_gcs_test, gcs_fixture, *tests::check_run_test_decorator("ENABLE_GCP_STORAGE_TEST", true)) {
    return test_env::do_with_async([](test_env& env) { tombstone_purge(env); }, test_env_config{.storage = make_test_object_storage_options("GS")});
}

void mv_tombstone_purge(test_env& env) {
    BOOST_REQUIRE(smp::count == 1);
        // In a column family with gc_grace_seconds set to 0, check that a tombstone
        // is purged after compaction.
        auto builder = schema_builder("tests", "tombstone_purge")
...
SEASTAR_TEST_CASE(mv_tombstone_purge_test) {
    return test_env::do_with_async([](test_env& env) { mv_tombstone_purge(env); });
}

SEASTAR_TEST_CASE(mv_tombstone_purge_s3_test, *boost::unit_test::precondition(tests::has_scylla_test_env)) {
    return test_env::do_with_async([](test_env& env) { mv_tombstone_purge(env); }, test_env_config{.storage = make_test_object_storage_options("S3")});
}
...
SEASTAR_FIXTURE_TEST_CASE(mv_tombstone_purge_gcs_test, gcs_fixture, *tests::check_run_test_decorator("ENABLE_GCP_STORAGE_TEST", true)) {
    return test_env::do_with_async([](test_env& env) { mv_tombstone_purge(env); }, test_env_config{.storage = make_test_object_storage_options("GS")});
}

void sstable_rewrite(test_env& env) {
    BOOST_REQUIRE(smp::count == 1);
        auto s = schema_builder(some_keyspace, some_column_family)
                .with_column("p1", utf8_type, column_kind::partition_key)
                .with_column("c1", utf8_type, column_kind::clustering_key)
...
SEASTAR_TEST_CASE(sstable_rewrite_test) {
    return test_env::do_with_async([](test_env& env) { sstable_rewrite(env); });
}

SEASTAR_TEST_CASE(sstable_rewrite_s3_test, *boost::unit_test::precondition(tests::has_scylla_test_env)) {
    return test_env::do_with_async([](test_env& env) { sstable_rewrite(env); }, test_env_config{.storage = make_test_object_storage_options("S3")});
}
...
SEASTAR_FIXTURE_TEST_CASE(sstable_rewrite_gcs_test, gcs_fixture, *tests::check_run_test_decorator("ENABLE_GCP_STORAGE_TEST", true)) {
    return test_env::do_with_async([](test_env& env) { sstable_rewrite(env); }, test_env_config{.storage = make_test_object_storage_options("GS")});
}


void sstable_max_local_deletion_time_2(test_env& env) {
...
SEASTAR_TEST_CASE(test_sstable_max_local_deletion_time_2) {
    return test_env::do_with_async([](test_env& env) { sstable_max_local_deletion_time_2(env); });
}
...
void min_max_clustering_key_2(test_env& env) {
...
SEASTAR_TEST_CASE(min_max_clustering_key_test_2) {
    return test_env::do_with_async([](test_env& env) { min_max_clustering_key_2(env); }, {});
}
...
SEASTAR_TEST_CASE(test_sstables_excluding_staging_correctness_s3, *boost::unit_test::precondition(tests::has_scylla_test_env)) {
