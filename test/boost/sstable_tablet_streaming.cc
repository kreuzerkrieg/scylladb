...
SEASTAR_TEST_CASE(test_streaming_ranges_distribution) {
    return test_env::do_with_async([](test_env& env) {
        auto get_tablet_sstable_collection = [](dht::token_range&& tablet_range) {
            return std::vector{tablet_sstable_collection{.tablet_range = std::move(tablet_range)}};
        };
        ...
        {
            auto collection = get_tablet_sstable_collection({dht::token{50}, dht::token{100}});
        ...
        {
            auto collection = get_tablet_sstable_collection({dht::token{1000}, dht::token{2000}});
            auto ssts = make_sstables_with_ranges(env,
                                                  {
                                                      {100, 200},
                                                      {300, 400},
        ...
        {
            auto collection = get_tablet_sstable_collection({dht::token{4}, dht::token{5}});
            auto ssts = make_sstables_with_ranges(env,
                                                  {
                                                      {0, 5},
                                                      {0, 3},
...
SEASTAR_TEST_CASE(test_streaming_ranges_distribution_in_tablets) {
    return test_env::do_with_async([](test_env& env) {
        auto get_tablet_sstable_collection = [](auto&&... tablet_ranges) {
            return std::vector<tablet_sstable_collection>{tablet_sstable_collection{.tablet_range = std::move(tablet_ranges)}...};
        };
