...
std::vector<shared_sstable> make_sstables_with_ranges(test_env& env, const std::vector<std::pair<int64_t, int64_t>>& ranges) {
    ...
    for (const auto& [first, last] : ranges) {
        ...
        test(sst).set_first_last(dht::decorated_key(dht::token{first}, partition_key(std::vector<bytes>{"1"})),
...
SEASTAR_TEST_CASE(test_streaming_ranges_distribution) {
    return test_env::do_with_async([](test_env& env) {
        // 0) The original problem, would break the loop in the [5..10) tablet
        {
            dht::token_range tablet_range{dht::token{0}, dht::token{5}};
            auto ssts = make_sstables_with_ranges(env,
                                                  {
                                                      {0, 5},  // fully contained
                                                      {5, 10}, // not contained, would break the loop as of https://github.com/scylladb/scylladb/issues/26979
                                                      {0, 10}, // partially contained
                                                  });
            auto&& [full, partial] = get_sstables_by_tablet_range(ssts, tablet_range);
            BOOST_REQUIRE_EQUAL(full.size(), 1);
        ...
        // 1) Exact boundary equality: SSTable == tablet
        {
            ...
            auto&& [full, partial] = get_sstables_by_tablet_range(ssts, tablet_range);
            BOOST_REQUIRE_EQUAL(full.size(), 1);
            BOOST_REQUIRE_EQUAL(partial.size(), 0);
        }
        ...
        // 2) Single-point overlaps at start/end
        {
            ...
            auto&& [full, partial] = get_sstables_by_tablet_range(ssts, tablet_range);
            BOOST_REQUIRE_EQUAL(full.size(), 0);
            BOOST_REQUIRE_EQUAL(partial.size(), 2);
        }
        ...
        // 3) Tablet fully inside a large SSTable
        {
            ...
            auto&& [full, partial] = get_sstables_by_tablet_range(ssts, tablet_range);
            BOOST_REQUIRE_EQUAL(full.size(), 0);
            BOOST_REQUIRE_EQUAL(partial.size(), 1);
        }
        ...
        // 4) Multiple SSTables fully contained in tablet
        {
            ...
            auto&& [full, partial] = get_sstables_by_tablet_range(ssts, tablet_range);
            BOOST_REQUIRE_EQUAL(full.size(), 3);
            BOOST_REQUIRE_EQUAL(partial.size(), 0);
        }
        ...
        // 6) Unsorted input (helper sorts) + mixed overlaps
        {
            ...
            auto&& [full, partial] = get_sstables_by_tablet_range(ssts, tablet_range);
            BOOST_REQUIRE_EQUAL(full.size(), 2);
            BOOST_REQUIRE_EQUAL(partial.size(), 2);
        }
