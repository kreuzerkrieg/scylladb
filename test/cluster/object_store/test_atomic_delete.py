...
# Extra config merged on top of what cluster_with_object_storage already sets
# (object_storage_endpoints + experimental_features).
_EXTRA_CFG = {'enable_user_defined_functions': False}
...
async def test_crash_after_compaction_prepare(manager: ManagerClient, cluster_with_object_storage):
    ...
    """
    async with cluster_with_object_storage(num_nodes=1, extra_cfg=_EXTRA_CFG) as (object_storage, servers):
        server = servers[0]
        cql = manager.get_cql()
...
@pytest.mark.asyncio
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_crash_after_compaction_unlink(manager: ManagerClient, cluster_with_object_storage):
...
@pytest.mark.asyncio
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_crash_during_truncate(manager: ManagerClient, cluster_with_object_storage):
...
@pytest.mark.asyncio
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_crash_before_batch_mutation_commits(manager: ManagerClient, cluster_with_object_storage):
