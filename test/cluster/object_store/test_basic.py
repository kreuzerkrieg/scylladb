...
async def test_basic(manager: ManagerClient, cluster_with_object_storage, tmp_path, mode, replication_factor):
    ...
    property_file = [{"dc": "dc1", "rack": f"r{i}"} for i in range(replication_factor)]

    async with cluster_with_object_storage(num_nodes=replication_factor,
                                           extra_cfg=cfg,
                                           property_file=property_file) as (object_storage, servers):
        cql = manager.get_cql()
...
async def test_garbage_collect(manager: ManagerClient, cluster_with_object_storage):
    ...
    async with cluster_with_object_storage(num_nodes=1, extra_cfg=cfg, cmdline=cmd) as (object_storage, servers):
        server = servers[0]
        cql = manager.get_cql()
...
async def test_create_keyspace_after_config_update(manager: ManagerClient, cluster_with_object_storage):
    async with cluster_with_object_storage(num_nodes=1, configure_endpoints=False) as (object_storage, servers):
        server = servers[0]
...
async def test_repair_creates_registry_entries(manager: ManagerClient, cluster_with_s3_storage):
    ...
    async with cluster_with_s3_storage(num_nodes=2, extra_cfg=cfg) as (s3_storage, servers):
...
async def test_registry_cleanup_on_all_nodes(manager: ManagerClient, cluster_with_object_storage, operation):
    ...
    async with cluster_with_object_storage(num_nodes=2,
                                           property_file=[{"dc": "dc1", "rack": "r0"},
                                                          {"dc": "dc1", "rack": "r1"}]) as (object_storage, servers):
        cql = manager.get_cql()
        hosts = await wait_for_cql_and_get_hosts(cql, servers, time.time() + 60)
