...
async def test_scaling(manager: ManagerClient, cluster_with_object_storage):
    ...
    property_file = [{"dc": "dc1", "rack": f"r{rack}"} for rack in range(num_racks)]

    async with cluster_with_object_storage(num_nodes=num_racks,
                                           extra_cfg={'enable_user_defined_functions': False},
                                           cmdline=cmdline,
                                           property_file=property_file) as (object_storage, servers):
        # cfg used below to add more nodes wired to the same object-storage endpoint.
        cfg = {
