...
async def test_node_failure_during_tablet_migration(manager: ManagerClient, tablet_storage, fail_replica, fail_stage, failure_detector_timeout):
    ...
    if fail_stage == 'revert_migration' and fail_replica == 'source' and tablet_storage is not None and tablet_storage.type == 'GS':
        skip_env('GCS flavor of this test gets stuck, deeper investigation is needed')
    if fail_stage == 'use_new' and fail_replica == 'destination' and tablet_storage is not None and tablet_storage.type == 'GS':
        skip_env('GCS flavor of this test gets stuck, deeper investigation is needed')

    logger.info("Bootstrapping cluster")
    cfg = make_cfg(tablet_storage, extra={'failure_detector_timeout_in_ms': failure_detector_timeout})
    host_ids = []
    servers = []
