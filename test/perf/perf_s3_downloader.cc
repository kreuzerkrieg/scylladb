...
struct round_stats {
    ...
    // refusing. The last two prove the controller was actually in the loop:
    // it starts disabled, so a run where no shard ever engaged measured the
    // unmitigated behaviour whatever else the numbers say.
    unsigned throttle_exhaustions = 0;
    uint64_t retry_quota_denials = 0;
    unsigned limiters_engaged = 0; // shards whose controller left the disabled state
    double fill_rate = 0.0;        // summed admitted requests/sec across shards
    ...
    round_stats& operator+=(const round_stats& o) noexcept {
        ...
        limiters_engaged += o.limiters_engaged;
...
public:
    ...
    round_stats collect_stats() const noexcept {
        ...
        return {
            ...
            .limiters_engaged = (_limiter && _limiter->enabled()) ? 1u : 0u,
            .fill_rate = _limiter ? _limiter->fill_rate() : 0.0,
    ...
    return app.run(argc, argv, [&app]() -> future<> {
        ...
        if (mode == "upload") {
            ...
            plog.warn("RESULT {{\"mode\":\"upload\",\"shards\":{},\"elapsed_sec\":{:.1f},"
                      ...
                      "\"throttle_exhaustions\":{},\"retry_quota_denials\":{},\"limiters_engaged\":{},\"fill_rate\":{:.1f}}}",
                      this_smp_shard_count(),
                      elapsed,
                      total.read_requests,
                      puts,
