...
class InjectingHandler(BaseHTTPRequestHandler):
    ...
    def __init__(self, server_type, policies, logger, uri, max_retries, *args, **kwargs):
    ...
    def process_request(self):
        try:
            ...
            if policy.should_fail:
                ...
                elif self.server_type == "GCS":
                    self.respond_gcp_with_error(reset_connection=policy.server_should_fail)
            else:
...
class ObjectStorageProxyServer:
    def __init__(self, server_type: str, host: str, port: int, uri: str, max_retries: int, seed: int, logger):
        self.server_type = server_type.upper()
        self.logger = logger
        self.logger.info('Setting proxy random seed to %s', seed)
        random.seed(seed)
        self.req_states = LRUCache(10000)
        handler = partial(InjectingHandler, server_type, self.req_states, logger, uri, max_retries)
