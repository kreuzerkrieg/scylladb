...
# the callbacks.
#
# Parking binds the callback to the loop it was registered on.  A cluster is
# recycled on the ScyllaClusterManager's own loop, in its own thread, while the
# callbacks own objects the fixtures created on a different loop -- a
# subprocess handle awaited from the wrong loop hangs instead of failing.  What
# comes out of the table is therefore a plain awaitable that is safe to await
# from anywhere, so the caller does not need to know which loop is underneath.
_parked_teardown_callbacks: dict[int, Callable[[], Awaitable[None]]] = {}
_teardown_callback_handles = itertools.count(1)


def _bind_to_current_loop(callback: Callable[[], Any]) -> Callable[[], Awaitable[None]]:
...
def park_teardown_callback(callback: Callable[[], Any]) -> int:
    ...
    _parked_teardown_callbacks[handle] = _bind_to_current_loop(callback)
    return handle


def unpark_teardown_callback(handle: int) -> None:
    """Drop a parked callback whose registration did not go through."""
    _parked_teardown_callbacks.pop(handle, None)


async def fire_teardown_callback(handle: int) -> None:
    """Resolve a parked callback and await it, from whichever loop fires it."""
    invoke = _parked_teardown_callbacks.pop(handle, None)
    if invoke is None:
        raise KeyError(f"teardown callback {handle} was already fired or never parked")
    await invoke()
...
class ScyllaCluster:
    ...
    async def run_teardown_callbacks(self) -> None:
        ...
        while self.teardown_callbacks:
            ...
            try:
                await fire_teardown_callback(handle)
            except Exception as e:
