...
void client::group_client::register_metrics(std::string class_name, std::string host) {
    ...
    defs.emplace_back(sm::make_counter("total_read_requests", [this] { return stats[httpd::GET].ops; },
    ...
    defs.emplace_back(sm::make_counter("total_write_requests",
            [this] { return stats[httpd::PUT].ops; },
            sm::description("Total number of object write requests"), {ep_label, sg_label}));
    defs.emplace_back(sm::make_counter("total_read_bytes", [this] { return stats[httpd::GET].bytes; },
    ...
    defs.emplace_back(sm::make_counter("total_write_bytes",
            [this] { return stats[httpd::PUT].bytes; },
            sm::description("Total number of bytes written to objects"), {ep_label, sg_label}));
    defs.emplace_back(sm::make_counter("total_read_latency_sec", [this] { return stats[httpd::GET].duration.count(); },
    ...
    defs.emplace_back(sm::make_counter("total_write_latency_sec",
            [this] { return stats[httpd::PUT].duration.count(); },
            sm::description("Total time spent writing data to objects"), {ep_label, sg_label}));
    ...
    for (size_t i = 0; i < http_method_count; ++i) {
        ...
        defs.emplace_back(sm::make_counter(fmt::format("total_{}_requests", lower_method_name),
                [this, method] { return stats[method].ops; },
                sm::description(fmt::format("Total number of HTTP {} requests", method_name)), {ep_label, sg_label, ml}));
        defs.emplace_back(sm::make_counter(fmt::format("total_{}_bytes", lower_method_name),
                [this, method] { return stats[method].bytes; },
                sm::description(fmt::format("Total number of bytes transferred via HTTP {}", method_name)), {ep_label, sg_label, ml}));
        defs.emplace_back(sm::make_counter(fmt::format("total_{}_latency_sec", lower_method_name),
                [this, method] { return stats[method].duration.count(); },
                sm::description(fmt::format("Total time in seconds spent in HTTP {} requests", method_name)), {ep_label, sg_label, ml}));
        defs.emplace_back(sm::make_counter(fmt::format("total_{}_retries", lower_method_name),
                [this, method] { return stats[method].retries; },
                sm::description(fmt::format("Total number of HTTP {} retries", method_name)), {ep_label, sg_label, ml}));
    }
...
http::client::reply_handler client::wrap_handler(http::request& request,
                                                               ...
                                                               io_stats& stats) {
    return [this, &request, expected, handler = std::move(handler), &stats, first_attempt = true](const http::reply& rep, input_stream<char>&& in) mutable -> future<> {
        if (!first_attempt) {
            ++stats.retries;
        }
        first_attempt = false;
...
future<> client::make_request(http::request req,
    ...
    auto method = httpd::str2type(request._method);
    co_await authorize(request);
    auto& gc = co_await find_or_create_client();
    auto& method_stats = gc.stats[method];
    ++method_stats.ops;
    auto handler = wrap_handler(request, std::move(handle), expected, method_stats);
...
future<temporary_buffer<char>> client::get_object_contiguous(sstring object_name, range download_range, seastar::abort_source* as) {
    ...
    co_await make_request(std::move(req), [&off, &ret, &object_name, start = s3_clock::now()] (group_client& gc, const http::reply& rep, input_stream<char>&& in_) mutable -> future<> {
        ...
        co_await in.consume([&off, &ret] (temporary_buffer<char> buf) mutable {
            ...
            gc.stats[httpd::GET].update(off, s3_clock::now() - start);
        });
    }, expected, as);
    ret->trim(off);
...
future<> client::put_object(sstring object_name, temporary_buffer<char> buf, seastar::abort_source* as) {
    ...
    co_await make_request(std::move(req), [len, start = s3_clock::now()] (group_client& gc, const auto& rep, auto&& in) {
        gc.stats[httpd::PUT].update(len, s3_clock::now() - start);
...
future<> client::multipart_upload::upload_part(memory_data_sink_buffers bufs) {
    ...
    (void)_client->make_request(std::move(req), [this, size, part_number, start = s3_clock::now()] (group_client& gc, const http::reply& rep, input_stream<char>&& in_) mutable -> future<> {
        ...
        _part_etags[part_number] = std::move(etag);
        gc.stats[httpd::PUT].update(size, s3_clock::now() - start);
        return make_ready_future<>();
            ...
            try {
                ...
                co_await _client->make_request(
                    ...
                    [this, start = s3_clock::now(), pf_length = current_range.length()](group_client& gc, const http::reply& reply, input_stream<char>&& in_) mutable -> future<> {
                        ...
                        gc.stats[httpd::GET].duration += s3_clock::now() - start;
                        ...
                        while (_buffers_size < _max_buffers_size && !_is_finished) {
                            ...
                            gc.stats[httpd::GET].bytes += buff_size;
                            _range += buff_size;
...
class client::greedy_download_source final : public seastar::data_source_impl {
    struct content_range {
        uint64_t start;
        uint64_t end;
        uint64_t total;
                    ...
                    [this, start = s3_clock::now()](group_client& gc, const http::reply& reply, input_stream<char>&& in_) mutable -> future<> {
                        ...
                        while (!_is_finished) {
                            ...
                            utils::get_local_injector().inject("kill_s3_inflight_req", [] {
                                // Inject non-retryable error to emulate source failure
                                throw aws::aws_exception(
                                    aws::aws_error(aws::aws_error_type::RESOURCE_NOT_FOUND, "Injected ResourceNotFound", utils::http::retryable::no));
                            });

                            s3l.trace("Fiber for object '{}' will try to read within range {}", _object_name, _range);
                            _buffer = co_await in.read();
                            auto buff_size = _buffer.size();
                            gc.stats[httpd::GET].bytes += buff_size;
                            _range += buff_size;
                            if (buff_size == 0 && _range.length() == 0) {
                                s3l.trace("Fiber for object '{}' signals EOS", _object_name);
                        ...
                        // Re-arm _bg_fiber_cv if we broke out of the inner loop without signaling
                        // _get_cv (i.e., the HTTP response body ended but _range still has data).
                        // The token consumed by _bg_fiber_cv.wait() must be returned so the next
                        // outer-loop iteration can proceed without deadlocking: get() is still
                        // blocked on _get_cv.wait() and won't re-signal _bg_fiber_cv until it
                        // returns, but it can't return until the fiber signals _get_cv.
                        if (!_is_finished) {
                            _bg_fiber_cv.signal();
                        }
                        co_await in.close();
...
public:
    greedy_download_source(shared_ptr<client> cln, sstring object_name, range range, seastar::abort_source* as)
        : _client(std::move(cln)), _object_name(std::move(object_name)), _as(as), _range(range) {
        s3l.trace("Constructing greedy_download_source for object '{}'", _object_name);
        _filling_fiber = make_filling_fiber();
    }

    future<temporary_buffer<char>> get() override {
        if (_is_finished) {
            co_return std::move(_buffer);
        }
        _bg_fiber_cv.signal();
        co_await _get_cv.wait();
        co_return std::move(_buffer);
    ...
    future<> close() override {
        ...
        _bg_fiber_cv.broadcast();
        _get_cv.broadcast();
        s3l.trace("Closing greedy_download_source for object '{}'", _object_name);
        co_await std::move(_filling_fiber);
    }
};
...
data_source client::make_greedy_download_source(sstring object_name, range range, seastar::abort_source* as) {
    return data_source(std::make_unique<greedy_download_source>(shared_from_this(), std::move(object_name), range, as));
}
...
class client::do_upload_file : private multipart_upload {
    ...
    future<> upload_part(file f, uint64_t offset, uint64_t part_size, uint64_t part_number) {
        ...
        co_await _client->make_request(std::move(req), [this, part_size, part_number, start = s3_clock::now()] (group_client& gc, const http::reply& reply, input_stream<char>&& in_) mutable -> future<> {
            ...
            s3l.trace("uploaded {} part data -> etag = {} (upload id {})", part_number, etag, _upload_id);
            _part_etags[part_number] = std::move(etag);
            gc.stats[httpd::PUT].update(part_size, s3_clock::now() - start);
            return make_ready_future();
        }, http::reply::status_type::ok, _as).handle_exception([this, part_number] (auto ex) {
            s3l.warn("couldn't upload part {}: {} (upload id {})", part_number, ex, _upload_id);
        });
