...
client::group_client& client::find_or_create_client() {
    ...
    if (it == _https.end()) [[unlikely]] {
        ...
        s3l.debug("Creating S3 client for scheduling group '{}' (shares={}, connections_per_share={}): max_connections={}",
                  sg.name(), sg.get_shares(), _cfg->connections_per_share, max_connections);
