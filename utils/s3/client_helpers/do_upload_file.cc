...
seastar::file_input_stream_options do_upload_file::input_stream_options() {
...
future<> do_upload_file::copy_to(utils::input_stream<char> input,
...
future<> do_upload_file::upload_part(file f, uint64_t offset, uint64_t part_size) {
    ...
    std::ignore = _client
                      ->make_request(
                          ...
                          [this, part_size, part_number, start = s3_clock::now()](
                              client::group_client& gc, const http::reply& reply, input_stream<char>&& in_) mutable -> future<> {
...
std::pair<unsigned, size_t> do_upload_file::calc_part_size(size_t total_size, size_t part_size) {
    if (part_size > 0) {
...
future<> do_upload_file::multi_part_upload(file&& f, uint64_t total_size, size_t part_size) {
...
future<> do_upload_file::put_object(file&& f, uint64_t len) {
...
do_upload_file::do_upload_file(shared_ptr<client> cln,
