...
future<> multipart_upload::start_upload() {
...
future<> multipart_upload::upload_part(memory_data_sink_buffers bufs) {
    if (!upload_started()) {
...
future<> multipart_upload::abort_upload() {
...
future<> multipart_upload::finalize_upload() {
    ...
    if (parts_xml_len == 0) {
        co_await coroutine::return_exception(std::runtime_error("Cant parse ETag list, aborting multipart upload"));
    }
...
future<> multipart_upload::upload_part(std::unique_ptr<upload_sink> piece_ptr) {
    if (!upload_started()) {
        co_await start_upload();
    }
