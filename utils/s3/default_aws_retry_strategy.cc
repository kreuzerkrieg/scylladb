...
seastar::future<bool> default_aws_retry_strategy::should_retry(std::exception_ptr error, unsigned attempted_retries) const {
    ...
    if (attempted_retries >= _max_retries) {
        // Name the cause. Without it "Retries exhausted" says only that a request
        // died, and a run cannot tell an exhaustion caused by throttling from any
        // other kind -- which is the distinction the S3 throttling measurements
        // turn on.
